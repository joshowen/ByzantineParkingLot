...
//		jtp.addTab("Parking Lot", lotp);
	...
	private void makeMainPanel(){
		...
		JTextField avgtf = new JTextField(averageWaitTime(cars));    
		...
		JTextField wcartf = new JTextField(numberWaiting(cars) + " Cars");
	...
	private void makeGatePanel(){
		...
		JPanel ggp;
		ginfop = new JPanel(new FlowLayout(FlowLayout.CENTER, 25, 10));
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(Character.isDigit(e.getActionCommand().charAt(1))){
			if(e.getActionCommand().charAt(0) == 'G'){
				...
				JTextField ncartf = new JTextField(gr.queueSize + " Cars");
	...
	int numberWaiting(Vector<Car> cars){
		int number = 0;
		for (int i = 0; i < cars.size(); i++){
			Car c = cars.get(i);
			if(c.toGate != null && c.fromGate == null)
				number++;
		}
		return number;
	}
	...
	String averageWaitTime(Vector<Car> cars){
		String output = "";
		long totMins = 0;
		for(int i = 0; i < cars.size(); i++){
			totMins += (cars.get(i).fromGate.getTimeInMillis() - cars.get(i).toGate.getTimeInMillis()) / minLength / 60;
		}
		
		if(cars.size() != 0)
			totMins = totMins / cars.size();
